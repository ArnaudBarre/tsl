  ...
  ObjectExpression(path) {
    ...
    for (const p of path.node.properties) {
      if (p.type === "ObjectProperty" && p.key.type === "Identifier") {
        if (p.key.name === "data") {
          assert(p.value.type === "ObjectExpression", "data is not an object");
          dataProp = p;
...
traverse(testAST, {
  ...
  ObjectExpression(path) {
    ...
    if (codeProp) {
      ...
      const languageOptionsProp = getObjectValue(path.node, "languageOptions");
      ...
      if (
        ...
        && languageOptionsProp.value.properties[0].key.name === "parserOptions"
