...
export const messages = {
  unusedDefaultExport: "Unused default export",
  unusedNamedExport: "Unused named export",
};
  ...
  aggregate(context, files) {
    ...
    for (const file of files) {
      ...
      for (const named of file.data.exports.named) {
        const used = file.data.usages.find((u) => u.named.includes(named.name));
        if (!used) {
          context.report({
