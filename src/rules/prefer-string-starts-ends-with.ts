...
type Context = Infer<typeof preferStringStartsEndsWith>["Context"];
...
export const preferStringStartsEndsWith = createRule({
  ...
  visitor: {
    BinaryExpression(node, context) {
      ...
      // foo[0] === 'b'
      if (
        context.options.allowSingleElementEquality === "never" &&
      ...
      if (
        ...
        node.left.arguments.length === 1 &&
        node.left.arguments[0].kind === SyntaxKind.RegularExpressionLiteral &&
    ...
    CallExpression(node, context) {
      ...
      if (
        ...
        node.expression.expression.kind === SyntaxKind.RegularExpressionLiteral
      ) {
...
export const test = () =>
  ruleTester({
    rule: preferStringStartsEndsWith,
    valid: [
      `
      function f(s: string[]) {
        s[0] === 'a';
